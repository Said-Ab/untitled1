...
select avg(year_of_issue)::numeric(10,1)from cars;
...
select count(id)from cars where brand='Mercedes-Benz';
...
select * from  cars order by year_of_issue desc,id  limit 10  offset 4 ;
...
select color, count(*)as car_caount from cars group by color order by count(*) desc limit 1;
